    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card b-radius--10">
                        ...
                        <table class="table table--light style--two">
                            <thead>
                                <tr>
                                    ...
                                    <th>@lang('Settled At')</th>
                                    <th>@lang('Status')</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($settlement as $data)
                                    <tr>
                                        ...
                                        <td data-label="@lang('Order')">
                                            <a href="{{route('seller.order.details',$data->order_id)}}?ref={{ encrypt($data->order_ref) }}" class="badge btn-sm bg--primary approveBtn" data-admin_feedback="{{$data->admin_feedback}}">View Order <small>({{ __($data->order_ref) }})</small></a>
                                        ...
                                        <td data-label="@lang('Settled At')" class="text--success">
                                            <span class="d-block font-weight-bold">{{showDateTime($data->settled_at)}}</span> 
                                            <span class="d-block font-weight-bold">{{diffForHumans($data->settled_at)}}</span>
                                        </td>
                ...
                @if($settlement->hasPages())
                    <div class="card-footer py-4">
                        {{ paginateLinks($settlement) }}
                    </div>
                @endif
            </div>
        </div>
