...
class SellerController extends Controller
{
    ...
    public function home()
    {
        ...
        $pendingsettlement = Settlement::where('seller_id', seller()->id)->where('status', '=', 0)->sum('final_amount');

        return view('seller.dashboard', compact('pendingsettlement','pageTitle','order','sale','months','report','withdrawalMonth','withdraw','product','latestOrders'));
    }
    ...
    public function cashback()
    {
       ...
       // $cashbacks     = Cashback::whereSellerId($seller->id)->with('product')->with('store')->paginate(10);
        ...
        return view('seller.cashback', compact('pageTitle',  'cashbacks','sum'));
    }
    ...
    public function shopUpdate(Request $request)
    {
        ...
        $shop->cashback_amount             = $request->cashback_amount;
        $shop->cashback_type             = $request->cashback_type;
        $shop->cashback_status             = $request->cashback_status ? 1 : 0;;
    ...
    {
        $pageTitle = 'Settlement Setting';
        return view('seller.settlement.settings', compact('pageTitle'));
    }

    public function settlementSubmit(Request $request)
    {
        $this->validate($request, [
            'cycle' => 'required'
        ]);
        ...
        $seller = seller();
        $seller->settlement_cycle = $request->cycle;
        $seller->save();
        $notify[] = ['success', 'Settlement settings updated successfully.'];
        return back()->withNotify($notify);
    ...
    public function settlementLog()
    {
        ...
        $settlement = Settlement::when(request()->search, function($q){
            return $q->where('trx',request()->search);
        })->where('seller_id', seller()->id)->with('order')->orderBy('id','desc')->paginate(getPaginate());
        return view('seller.settlement.log', compact('pageTitle','settlement','emptyMessage'));
    }
