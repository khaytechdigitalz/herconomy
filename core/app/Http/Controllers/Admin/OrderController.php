        ...
        if($request->action==1){
            ...
            foreach($order->orderDetail as $detail) {
                ...
                if($detail->seller_id != 0){
                  ...
                  //START RUN SETTLEMENT\\
                  if($seller->settlement_cycle == 0)
                  {
                    ...
                    $settlement->charge = $commission;
                    $settlement->trx = getTrx();
                    $settlement->status = 1;
                    $settlement->final_amount = $finalAmount;
                    $settlement->after_charge = $finalAmount - $commission;
                    $settlement->save();
                  }
                  else
                  {
                    $SettlementDateTime = Carbon::now()->addDay($seller->settlement_cycle);
                    $settlement = new Settlement();
                    $settlement->order_ref = $detail->id;
                    $settlement->order_id = $order->id;
                    $settlement->seller_id = $detail->seller_id;
                    ...
                    $settlement->charge = $commission;
                    $settlement->trx = getTrx();
                    $settlement->settled_at = $SettlementDateTime;
                    $settlement->final_amount = $finalAmount;
                    $settlement->after_charge = $finalAmount - $commission;
                    $settlement->save();
                  }
