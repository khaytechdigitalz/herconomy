...
class DepositController extends Controller
{
    ...
    public function approve(Request $request)
    {
        ...
        $deposit->status = 1;
            ...
            $ref = User::whereId($deposit->user_id)->first();
            if($ref)
            {
                $rewardreferrer  = $deposit->amount * ($general->upline_reward / 100);
                $ref->balance += $rewardreferrer;
                $ref->save();
            }
                $rewardbuyer  = $deposit->amount * ($general->downline_reward / 100);
                $user->balance += $rewardbuyer;
                $user->save();
