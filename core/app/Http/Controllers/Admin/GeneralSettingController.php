...
class GeneralSettingController extends Controller
{
    ...
    {
        $tiers = Tier::all();
        $pageTitle = 'Account Tiers Setting';
        return view('admin.setting.tiers', compact('pageTitle','tiers'));
    }

    public function tiercreate(Request $request)
    {  
        $tiercreate = new Tier();
        $tiercreate->name = $request->name;
        $tiercreate->min = $request->min;
        $tiercreate->max = $request->max;
        $tiercreate->discount = $request->discount;
        $tiercreate->save();
        $notify[] = ['success', 'New account tier has been created successfuly.'];
        return back()->withNotify($notify);
    }
    public function tierupdate(Request $request)
    {  
        $tier = Tier::whereId($request->id)->firstOrFail();
        $tier->name = $request->name;
        $tier->min = $request->min;
        $tier->max = $request->max;
        $tier->discount = $request->discount;
        $tier->save();
        $notify[] = ['success', 'Account tier has been updated successfuly.'];
        return back()->withNotify($notify);
    }
    public function tierdelete(Request $request)
    {  
        $tier = Tier::whereId($request->id)->firstOrFail(); 
        $tier->delete();
        $notify[] = ['success', 'Account tier has been deleted successfuly.'];
        return back()->withNotify($notify);
    }
    ...
    public function update(Request $request)
    {
        ...
        $general->downline_reward = $request->downline_reward;
        $general->upline_reward = $request->upline_reward;
