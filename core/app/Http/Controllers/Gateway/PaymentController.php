...
class PaymentController extends Controller
{
    ...
    public static function userDataUpdate($trx)
    {
        ...
        if ($data->status == 0) {
            ...
            //$this->cashbackjoint($detail,$seller,$cashbacktype,$cashback_amount,$admin_fee);
            ...
            foreach($order->orderDetail as $detail) {
                ...
                try {
                    ...
                    if($seller->shop)
                    {
                        $cashbacktype = $seller->shop->cashback_type;
                        $cashbackstatus = $seller->shop->cashback_status;
                      
                        if($cashbacktype == 'percentage')
                        {
                            $cashback_calculated = ($detail->total_price * $seller->shop->cashback_amount)/100;
                            $admin_fee = ($cashback_calculated * $general->product_commission)/100;
                            $cashback_amount = $cashback_calculated - $admin_fee;
                        }
                        if($cashbacktype == 'flat')
                        {
                        $cashback_calculated = $seller->shop->cashback_amount;
                        $admin_fee = ($cashback_calculated * $general->product_commission)/100;
                        $cashback_amount = $cashback_calculated - $admin_fee; 
                        }
                        ...
                        $cashback->product_id = $detail->product_id;
                        $cashback->order_ref = $data->order_id;
                        $cashback->seller_id = $detail->seller_id;
                        $cashback->store_id = $seller->shop->id;
                        $cashback->amount = $detail->total_price;
                        $cashback->trx = getTrx();
                        $cashback->type = $cashbacktype;
                        $cashback->calc_cashback = $cashback_calculated;
                        $cashback->cashback = $cashback_amount;
                        $cashback->admin_fee = $admin_fee;
                        $cashback->status = 1;
                        $cashback->save();
                        }
                    ...
                    Log::warning('CASHBACK ERROR', ['user' => $exp]);

                }
            
                /*
                $fee  = ($detail->total_price * $general->product_commission)/100;
                $payable =  $detail->total_price - $fee;
                if($detail->seller_id != 0)
                {
                    $settlement = new Settlement();
                    $settlement->order_ref = $detail->id;
                    $settlement->order_id = $data->order_id;
                    $settlement->seller_id = $detail->seller_id;
                    $settlement->amount = $detail->total_price;
                    $settlement->charge = $fee;
                    $settlement->trx = getTrx();
                    $settlement->final_amount = $payable;
                    $settlement->after_charge = $payable - $fee;
                    $settlement->save();
                }
                */
    ...
    public function cashbackjoin($detail,$seller,$cashbacktype,$cashback_amount,$admin_fee)
    {

        $user = User::find($data->user_id);
        $cashback = new Cashbackjoint();
        $cashback->user_id = $user->id;
        $cashback->order_ref = $data->order_id;
        $cashback->seller_id = $detail->seller_id;
        $cashback->store_id = $seller->shop->id;
